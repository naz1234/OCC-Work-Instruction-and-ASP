...
1. Apply `migrations/0003_create_document_editor.sql`, `migrations/0004_create_document_overrides.sql`, and `migrations/0005_create_document_text_overrides.sql` to the same `occ-work-instructions` D1 database.
...
- Per-row rename controls for the document title and EDMS folder, saved in D1 for every device.
